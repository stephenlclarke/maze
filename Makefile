...
coverage:
	...
	@test_binary="$$(find .build -path '*MazePackageTests.xctest/Contents/MacOS/MazePackageTests' -type f -print -quit)"; \
	...
	"$(SWIFT_LLVM_COV)" export -format=lcov -instr-profile="$$profile" "$$test_binary" --sources Sources/MazeCore > coverage.lcov
